...
async function main() {
    const tx = await clientContract.logoutUser(ersterContract, joby, "test");
    console.log("The message: ", tx);
    console.log("We are waiting..");
    await tx.wait();
    const lastLogoutInfo = await clientContract.lastLogoutInfo();
    console.log("last Logout Info: ", lastLogoutInfo);
}
